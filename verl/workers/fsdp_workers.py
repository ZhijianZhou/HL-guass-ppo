...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    def _build_critic_model_optimizer(self, config: FSDPCriticConfig):
        ...
        from verl.utils.hl_gauss_model import load_hl_gauss_valuehead_model
        ...
        with init_context(), warnings.catch_warnings():
            ...
            use_hl_gauss = self.config.get("use_hl_gauss", False) or self.config.get("use_hl_gauss_critic", False)
            if use_hl_gauss:
                critic_module = load_hl_gauss_valuehead_model(
                    local_path,
                    torch_dtype,
                    critic_model_config,
                    config.model.get("trust_remote_code", False),
                    n_bins=self.config.get("n_bins", 101),
                    v_min=self.config.get("v_min", -1.0),
                    v_max=self.config.get("v_max", 1.0),
                    sigma=self.config.get("sigma", None),
                    v_head_init_strategy=self.config.get("v_head_init_strategy", "normal"),
                )
            else:
    ...
    def init_model(self):
        ...
        from verl.workers.critic import DataParallelPPOCritic, HLGaussDataParallelPPOCritic
        ...
        use_hl_gauss_critic = self.config.get("use_hl_gauss_critic", False) or self.config.get("use_hl_gauss", False)
        critic_cls = HLGaussDataParallelPPOCritic if use_hl_gauss_critic else DataParallelPPOCritic
