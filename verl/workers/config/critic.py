...
class CriticConfig(BaseConfig):
    ...
    # HL-Gauss critic configuration (minimal baseline support)
    use_hl_gauss: bool = False
    """Whether to use an HL-Gauss value head for histogram-based value prediction."""
    use_hl_gauss_critic: bool = False
    """Alias for use_hl_gauss to keep old experiment scripts working."""
    n_bins: int = 101
    """Number of histogram bins in the HL-Gauss critic."""
    v_min: float = -1.0
    """Minimum value support for the HL-Gauss critic."""
    v_max: float = 1.0
    """Maximum value support for the HL-Gauss critic."""
    sigma: Optional[float] = None
    """Gaussian target smoothing width. Defaults to 0.75 * bin width when unset."""
    v_head_init_strategy: str = "normal"
    """Initialization strategy for the HL-Gauss value head."""
