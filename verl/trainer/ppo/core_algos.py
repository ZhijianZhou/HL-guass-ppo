...
def compute_hl_gauss_value_loss(
    ...
    loss_agg_mode: str = "token-mean",
    eps: float = None,
    fp32_target: bool = True,
):
    if predicted_logits.dim() != 3:
        raise ValueError(f"predicted_logits must be 3D [B, L, C], got {tuple(predicted_logits.shape)}")
    if returns.dim() != 2:
        raise ValueError(f"returns must be 2D [B, L], got {tuple(returns.shape)}")
    if response_mask.dim() != 2:
        raise ValueError(f"response_mask must be 2D [B, L], got {tuple(response_mask.shape)}")
    if n_bins < 3:
        raise ValueError(f"n_bins must be >= 3, got {n_bins}")
    if not (v_min < v_max):
        raise ValueError(f"v_min must be < v_max, got v_min={v_min}, v_max={v_max}")

    batch_size, response_length, n_classes = predicted_logits.shape
    if n_classes != n_bins:
        raise ValueError(f"n_bins mismatch: logits last dim={n_classes}, n_bins={n_bins}")
    if returns.shape != (batch_size, response_length):
        raise ValueError(f"returns shape mismatch: expected {(batch_size, response_length)}, got {tuple(returns.shape)}")
    if response_mask.shape != (batch_size, response_length):
        raise ValueError(
            f"response_mask shape mismatch: expected {(batch_size, response_length)}, got {tuple(response_mask.shape)}"
        )

    logits_dtype = predicted_logits.dtype
    if eps is None:
        eps = 1e-6 if logits_dtype in (torch.float16, torch.bfloat16) else 1e-12

    bin_width = (v_max - v_min) / n_bins
    if sigma is None:
        sigma = float(bin_width * 0.75)
    if sigma <= 0:
        raise ValueError(f"sigma must be > 0, got {sigma}")

    target_dtype = torch.float32 if fp32_target else logits_dtype
    clipped_returns = returns.clamp(min=v_min, max=v_max).to(target_dtype)
    bin_centers = v_min + (torch.arange(n_bins, device=predicted_logits.device, dtype=target_dtype) + 0.5) * bin_width

    scalar_expanded = clipped_returns.unsqueeze(-1)
    squared_diff = (bin_centers - scalar_expanded) ** 2
    target_probs = torch.exp(-squared_diff / (2 * sigma**2))
    target_probs = target_probs / target_probs.sum(dim=-1, keepdim=True).clamp_min(eps)
    target_probs = target_probs.to(logits_dtype)

    log_probs = F.log_softmax(predicted_logits, dim=-1)
    token_ce = -(target_probs * log_probs).sum(dim=-1)
    vf_loss = agg_loss(loss_mat=token_ce, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
    return vf_loss
